...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        self.latest_attn_max = att.max()  # tensor scalar
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.init_method == 'gpt2':
            for pn, p in self.named_parameters():
                if pn.endswith('c_proj.weight'):
                    torch.nn.init.normal_(p, mean=0.0, std=0.02/math.sqrt(2 * config.n_layer))
        elif config.init_method == 'constant':
            for pn, p in self.named_parameters():
                if p.dim() >= 2:
                    torch.nn.init.normal_(p, mean=0.0, std=config.init_value)
    ...
    def configure_optimizers(self, optimizer_name, weight_decay, learning_rate, betas, device_type):
        ...
        if optimizer_name == 'muon' or optimizer_name == 'adamuon':
        ...
        if optimizer_name == 'muon' or optimizer_name == 'adamuon':
            ...
            if optimizer_name == 'muon':
                optimizer1 = Muon(twoD_params, lr=learning_rate, momentum=0.95, rank=rank, world_size=world_size, weight_decay=weight_decay)
                print(f"using Muon and AdamW")
            else:
